...
There is also a map editor spec at `documents/map-editor-spec.md`. It defines a dev-only tile map authoring tool — a Vite frontend at `/editor` backed by a standalone Hono API server on `localhost:3001` — that reads and writes map source files directly. Key facts to carry:
...
- Map files that participate in the editor contain `// @map-editor:start` / `// @map-editor:end` markers. The API reads and writes only what is between these markers; everything else in the file is untouched. The canonical read/write functions live in `editor/server/sceneParser.ts` — use them, don't reimplement the format. (Rationale: ADR 0003.)
- Map names are sourced from the world graph module via a `SCENE_NAMES` array. The create endpoint appends to this array; do not change its format or the regex that reads it will break.
- The editor has its own milestone build order tracked in `documents/map-editor-spec.md`. Consult it when working on editor tasks.
- Map files live in `src/world/maps/`. They contain the `TILES` array and `SPAWN_POINTS` inside the marker block, plus the `Tilemap` definition (spritesheet coords + solid flag) outside the markers. Scene files (`src/scenes/`) import from map files — the dependency flows downward from scenes to world, never the reverse. If you add a new tile type to the shared tile definitions, also add a matching entry to the map's `Tilemap.defs`.
