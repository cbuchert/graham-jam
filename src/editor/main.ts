...
const newSceneBtn = document.getElementById(
  "new-scene-btn",
) as HTMLButtonElement
const resizeBtn = document.getElementById("resize-btn") as HTMLButtonElement
const resizeDialog = document.getElementById(
  "resize-dialog",
...
const newSceneDialog = document.getElementById(
  "new-scene-dialog",
) as HTMLDialogElement
...
function computeCellSize(cols: number, rows: number): number {
  ...
  return Math.min(
    MAX_CELL,
    Math.max(MIN_CELL, Math.min(IDEAL_CELL, byWidth, byHeight)),
  )
}
...
function setMode(next: Mode) {
  ...
  modeSpawnBtn.classList.toggle("is-active", mode === "spawn")
  rebuildPanel()
}
...
function rebuildPanel() {
  ...
  paletteEl.querySelectorAll(":scope > *:not(.section-label)").forEach((el) => {
    el.remove()
  })
...
function buildTilePalette() {
  for (const def of TILE_REGISTRY) {
    ...
    swatch.className = "tile-btn__swatch"
...
function buildSpawnItem(
  ...
  item.className = `spawn-item${name === selectedSpawnName ? " spawn-item--active" : ""}`

  // Name — click selects, double-click starts inline rename
  const nameEl = document.createElement("span")
  nameEl.className = "spawn-item__name"
  ...
  coordEl.className = "spawn-item__coords"
  coordEl.textContent = `${pos.x},${pos.y}`

  // Delete
  const delBtn = document.createElement("button")
  delBtn.className = "spawn-item__del"
  delBtn.textContent = "✕"
  delBtn.title = "Delete"
...
function startRename(
  ...
  input.value = oldName
  input.className = "spawn-item__rename-input"
...
canvas.addEventListener("mousemove", (e) => {
  if (isPainting && mode === "paint") paintAt(e)
})
...
async function apiCreateScene(name: string, width: number, height: number) {
  ...
  if (!res.ok) {
    ...
    throw new Error(
      body.error ?? `POST /api/scene/${name}/create → ${res.status}`,
    )
  }
}
...
resizeDialog.addEventListener("click", (e) => {
  if (e.target === resizeDialog) resizeDialog.close()
})
...
rsConfirm.addEventListener("click", () => {
  ...
  if (!Number.isInteger(newWidth) || newWidth < 4) {
    rsError.textContent = "Width must be at least 4."
    rsError.hidden = false
    return
  }
  if (!Number.isInteger(newHeight) || newHeight < 4) {
    rsError.textContent = "Height must be at least 4."
    rsError.hidden = false
    return
  }
...
nsConfirm.addEventListener("click", async () => {
  ...
  if (!name) {
    showNsError("Name is required.")
    return
  }
  if (!NAME_RE.test(name)) {
    showNsError(
      "Name must start with a letter and contain only a–z, 0–9, _ or -.",
    )
    return
  }
  ...
  if (!Number.isInteger(width) || width < 4) {
    showNsError("Width must be at least 4.")
    return
  }
  if (!Number.isInteger(height) || height < 4) {
    showNsError("Height must be at least 4.")
    return
  }
